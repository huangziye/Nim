...
import com.hzy.uikit.business.contact.core.query.TextSearcher;
...
public class ContactSearch {
    ...
    public static final HitInfo hitInfoFriend(IContact contact, TextQuery query) {
        ...
        if (range != null) {
            return new HitInfo(HitInfo.Type.Name, name, range);
        }

        range = TextSearcher.indexOf(query.t9, account, query.text);

        if (range != null) {
            return new HitInfo(HitInfo.Type.Account, account, range);
        }
